...
# Audio
# NOTE: Apple Music returns several URL shapes for catalog items. Examples:
#   https://music.apple.com/us/album/album-name/12345?i=67890
#   https://music.apple.com/us/song/track-name/67890
#   https://music.apple.com/us/songs/track-name/67890
# The original regex only handled the first form (album/playlist/artist),
# which meant `/song` URLs coming from the Apple Music API search results
# were treated as invalid and never added to the parsing queue. We accept
# both `song` and `songs` here and normalise them to `track` below.
APPLE_MUSIC_URL_REGEX = re.compile(
    r'https?://music\.apple\.com/([a-z]{2})/'
    r'(?P<type>album|playlist|artist|song|songs)'
    r'(?:/(?P<title>[-a-z0-9]+))?/'
    r'(?P<id>[\w.-]+)'
    r'(?:\?i=(?P<track_id>\d+))?(?:&.*)?$'
)
...
def parse_url(url):
    ...
    if re.match(APPLE_MUSIC_URL_REGEX, url):
        ...
        # Apple Music "song" / "songs" URLs represent individual tracks.
        # We normalise those types to the internal "track" type so that the
        # parsing worker routes them through apple_music_get_track_metadata.
        if raw_type in ("song", "songs"):
            item_type = "track"
        else:
            item_type = raw_type

        # Album URLs that include a `?i=track_id` query string still have an
        # album path but refer to a specific track. In that case override the
        # ID and type accordingly.
...
def parsingworker():
    while True:
        try:
            # Block until there is an item to parse instead of busy-waiting.
            item_id, item = pop_next_parsing_item(block=True)
            if item is None:
                # Timeout or spurious wake-up; simply loop and wait again.
                continue
