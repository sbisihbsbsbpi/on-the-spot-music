...
from .runtimedata import get_logger, download_queue, download_queue_lock, account_pool, temp_download_path, acquire_next_download_item
...
class DownloadWorker(QObject):
    ...
    def run(self):
        while self.is_running:
            try:
                try:
                    # Block briefly waiting for work instead of busy-waiting
                    # on an empty download_queue.
                    item = acquire_next_download_item(block=True, timeout=0.2)
                    if item is None:
                        # No work available yet; loop and re-check is_running.
                        continue
                    ...
                    # Stealth Mode: Check rate limits and optional session breaks for Apple Music
                    if item_service == "apple_music":
                        ...
                        # Check if we need a session break. When triggered, show a
                        # live countdown similar to the per-track delay so the user
                        # can see the remaining break time decreasing.
                        needs_break, break_duration = check_session_break()
                        if needs_break:
                            logger.info(
                                f"Stealth mode: Taking session break for {break_duration/60:.1f} minutes"
                            )

                            remaining = int(break_duration)
                            while remaining > 0:
                                if self.gui:
